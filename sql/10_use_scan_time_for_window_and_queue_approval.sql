-- ============================================================
-- StageLink — Use real scan time for attendance decisions
-- Ensures offline sync uses the exact scan timestamp, not sync time.
-- ============================================================

-- 1) Submit RPC: use p_scanned_local_at for window checks and writes
...
-- 2) Admin approval: apply scan event time from queue to final table
...
BEGIN
  ...
  v_scan_at := COALESCE(v_item.scanned_local_at, v_item.received_at, NOW());
  ...
  IF COALESCE(v_item.event_type, 'check_in') = 'check_in' THEN
    ...
    ELSE
      UPDATE public.practical_attendance
      SET check_in_at = COALESCE(check_in_at, v_scan_at),
          scanned_at = COALESCE(scanned_at, v_scan_at),
